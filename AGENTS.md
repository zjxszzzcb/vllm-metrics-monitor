...
- **Daemon threads must never die silently**: `scraper_loop` and `cleanup_loop` wrap their whole loop body in `try/except Exception`, log the traceback, and keep going — an unhandled error in a daemon thread kills it without any visible symptom (a source's dashboard quietly freezing is how this showed up). Preserve this when adding background threads. SQLite connections use `timeout=30` so concurrent writers survive WAL lock waits.
